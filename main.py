...
@app.get("/admin/test-whatsapp")
async def admin_test_whatsapp(phone: str = "573105870328", message: str = ""):
    """Send a test WhatsApp message via SonIA Agent."""
    wa = modules.get("whatsapp")
    if not wa:
        raise HTTPException(status_code=503, detail="WhatsApp module not available")
    
    if not message:
        message = (
            "Hola! Este es un mensaje de prueba de SonIA Core.\n\n"
            "Si recibes esto, la integracion SonIA Core -> SonIA Agent -> WhatsApp esta funcionando correctamente.\n\n"
            f"Timestamp: {datetime.now(COT).isoformat()}"
        )
...
async def admin_test_flow(phone: str = "573105870328"):
    """Run a quick test: scan DynamoDB, build mini report, send via WhatsApp."""
    dynamo = modules.get("dynamo")
    wa = modules.get("whatsapp")
    
    if not dynamo:
        raise HTTPException(status_code=503, detail="DynamoDB module not available")
    
    try:
        items = dynamo.scan_table()
        tenant_items = [i for i in items if str(i.get("tenant", {}).get("N", "")) == "1"]
        
        if not tenant_items:
            return {"status": "no_data", "message": "No shipments found for tenant 1 (BloomsPal)"}
        
        report_lines = ["*Reporte de Prueba - BloomsPal*\n"]
        report_lines.append(f"Total envios encontrados: {len(tenant_items)}\n")
        
        for idx, item in enumerate(tenant_items[:5]):
            order_num = item.get("orderNumber", {}).get("S", "N/A")
            status = item.get("status", {}).get("S", "N/A")
            tracking = ""
            packages = item.get("packages", {}).get("L", [])
            if packages:
                pkg = packages[0].get("M", {})
                tracking = pkg.get("trackingNumber", {}).get("S", "N/A")
            report_lines.append(f"\n{idx+1}. Orden #{order_num}")
            report_lines.append(f"   Estado: {status}")
            if tracking:
                report_lines.append(f"   Tracking: {tracking}")
        
        report_lines.append(f"\n\nGenerado: {datetime.now(COT).strftime('%Y-%m-%d %H:%M COT')}")
        report = "\n".join(report_lines)
        
        if wa:
            result = await wa.send_message(phone, report)
            return {"status": "sent", "phone": phone, "items": len(tenant_items), "report_preview": report[:300]}
        else:
            return {"status": "generated_not_sent", "report": report, "items": len(tenant_items)}
    except Exception as e:
